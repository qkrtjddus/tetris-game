    ...
    {		                   
    	...
    	if (fCurrPiece != null && fPlaying == true && fPaused == false) 
    	{
    	    if (direction == TetrisPiece.DOWN || direction == TetrisPiece.FALL)
            {
                ...
                if (fCurrPiece.move(direction) == false) fCurrPiece = null;
                else result = true;
            }
    ...
    private class GameThread extends Thread
    {
        public void run()
        {
            while(fPlaying)
            {
                    ...
                    if (fCurrPiece == null) 
                    {
                        ...
                        for (int rows = fBoard.getRows() - 1; rows >= 0; rows--) 
                        {
                            ...
                            if (same) 
                            {
                                ...
                                if (fTotalLines == 10) fDelay = 400;
                                if (fTotalLines == 20) fDelay = 300;
                ...
                if (fCurrPiece != null) try { sleep(fDelay); } catch (InterruptedException e) { System.err.println("Exception e: " + e); }
            }
        }
    }
